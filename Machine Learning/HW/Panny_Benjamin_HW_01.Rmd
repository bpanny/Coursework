---
title: "INFSCI 2595 Fall 2022 Homework: 01"
subtitle: "Assigned August 31, 2022; Due: September 7, 2022"
author: "Benjamin Panny"
date: "Submission time: September 7, 2022 at 11:00PM EST"
...
Discuss your conclusions here: The range of diamond prices for all colors appears to be the same, as well as the first quartiles. On the other hand, the median and third quartile prices as well as the IQR of prices from D-J show a trend of increasing. As such, the number of outliers in each color category shows a decreasing trend from color D to color J.
...
diamonds %>% 
  ggplot(mapping = aes(x = color, y = price)) +
  geom_boxplot(mapping = aes(fill = carat > median(carat))) + facet_grid(clarity ~ cut) + 
...
diamonds %>% 
  ...
  ggplot(mapping = aes(x = color, y = price)) +
  geom_boxplot(mapping = aes(fill = carat > median(diamonds$carat))) + facet_grid(clarity ~ cut) + 
  theme(legend.position = "top")
```
...
Type your discussion here: In figure 2b, there was an uptrend of median and 3rd quartile prices. In figure 2f, the median prices are largely the same for each color when carat is less than the median of all diamonds in the dataset. When carat value is greater than the carat median of the dataset, the median prices show a downtrend from color D to J. In figure 2b, the IQR of prices from D to J appeared to increase. In figure 2f, the IQR for each color is largely the same when carat is less than the carat median of the dataset. When carat is greater than median carat, the IQR for each color increases from D to J, as in figure 2b.
...
Yes, the displ variable is significant because it's confidence interval does not overlap with zero.
...
There are 4 coefficients, all of which are significantly different than zero at the presented confidence level because their confidence intervals do not overlap with zero.
