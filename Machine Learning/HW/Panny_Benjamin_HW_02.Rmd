---
title: "INFSCI 2595 Fall 2022 Homework: 02"
subtitle: "Assigned September 8, 2022; Due: September 15, 2022"
author: "Benjamin Panny"
date: "Submission time: September 15, 2022 at 11:00PM EST"
...
**IMPORTANT**: You are working with 3 data sets in this assignment. This does not mean you must always have multiple "versions" of a data set to complete a machine learning application. This assignment is constructed so that you may experience the relationship between sample size and noise on the ability to learn and identify appropriate levels of complexity. The data in this assignment are synthetic data created specifically for this assignment.  
...
```{r, solution_01a}
df_low_high %>%
  ggplot(mapping = aes(x=x, y=y)) + geom_point(size=3) + geom_smooth(method=lm) + geom_smooth()
```

**Is the relationship linear or non-linear?**  

The relationship may be interpreted as both linear and non-linear, as indicated by the geom_smooth() function capturing a fair amount of data using either method. However, it seems that the point with a value of x > 2 and y > 30 has a large influence on the shape of the non-linear relationship.
...
fit_lm_1 <- lm(y ~ x, df_low_high)
```


```{r, solution_01b_b, eval=TRUE}
### quadratic relationship (degree 2)
fit_lm_2 <- lm(y ~ x + I(x^2), df_low_high)
```


```{r, solution_01b_c, eval=TRUE}
### cubic relationship (degree 3)
fit_lm_3 <- lm(y ~ x + I(x^2) + I(x^3), df_low_high)
```


```{r, solution_01b_d, eval=TRUE}
### degree 4
fit_lm_4 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4), df_low_high)
```


```{r, solution_01b_e, eval=TRUE}
### degree 5
fit_lm_5 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5), df_low_high)
```


```{r, solution_01b_f, eval=TRUE}
### degree 6
fit_lm_6 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6), df_low_high)
```


```{r, solution_01b_g, eval=TRUE}
### degree 7
fit_lm_7 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7), df_low_high)
```


```{r, solution_01b_h, eval=TRUE}
### degree 8
fit_lm_8 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), df_low_high)
```


```{r, solution_01b_i, eval=TRUE}
### degree 9
fit_lm_9 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8) + I(x^9), df_low_high)
```
...
```{r, solution_01c}
tr.rmse <- tibble(rmse(fit_lm_1, df_low_high), rmse(fit_lm_2, df_low_high), 
                  rmse(fit_lm_3, df_low_high), rmse(fit_lm_4, df_low_high), 
                  rmse(fit_lm_5, df_low_high), rmse(fit_lm_6, df_low_high), 
                  rmse(fit_lm_7, df_low_high), rmse(fit_lm_8, df_low_high), 
...
```{r, solution_01d}
tr.rsquare <- tibble(rsquare(fit_lm_1, df_low_high), rsquare(fit_lm_2, df_low_high), 
                  rsquare(fit_lm_3, df_low_high), rsquare(fit_lm_4, df_low_high), 
                  rsquare(fit_lm_5, df_low_high), rsquare(fit_lm_6, df_low_high), 
                  rsquare(fit_lm_7, df_low_high), rsquare(fit_lm_8, df_low_high), 
...
According to the training set performance metrics RMSE and R-squared, the 9th order polynomial model is the best fit. This is because it has the highest R-squared value and the lowest RMSE value. Therefore the two performance metrics agree.
...
For the intercept coefficient, the estimates partially overlap in the 2nd order, 6th order, and 9th order polynomial models, but only in the 2nd order model is the intercept coefficient statistically significantly less than 0. Similar is true for the 1st order and 2nd order coefficients in the three models, except for these coefficients being statistically significantly greater than 0. The uncertainty in all three coefficient estimates increases from the 2nd order polynomial, to the 6th order polynomial, to the 9th order polynomial model

In the coefficients present in only the 6th and 9th order model, the location of the estimates are sometimes the same (e.g., cubic term) and sometimes not (e.g., 4th order term). However, the location and scale of the remaining coefficients up to the 6th order term for both the 6th order and 9th order models both make it appear as if the coefficients from neither model are significantly different from 0.
...
df_viz_1 %>% 
  ggplot(mapping = aes(x = x)) +
  geom_point(mapping = aes(y = y), data = df_low_high, shape = 0, color = "red") +
  geom_line(mapping = aes(y = pred_trend), size = 1)
```
...
df_viz_all %>%
  ggplot(mapping = aes(x = x)) +
  geom_point(mapping = aes(y = y), data = df_low_high) + 
  geom_line(mapping = aes(y = pred_trend)) +
  facet_wrap(~poly_degree, scales = "free_y")
```
...
The predictive trends of the best performing model (the 9th order polynomial) seem problematic between the x values of approximately 1.5 to 2.6. This is because the predictive trend extends to extreme values of y between x values of approximately 1.5 to 2.6. Therefore, I do not believe the trend is consistent with the training set.
...
set.seed(202214)
id_train <- sample(1:nrow(df_low_high), .8*nrow(df_low_high))
...
ggplot(mapping = aes(x = x, y = y)) +
  geom_point(data = df_train, color = "red", shape = 1, size = 4) +
  geom_point(data = df_holdout, color = "blue", shape = 2, size = 4)
```
...
### linear relationship (degree 1)
fit_train_1 <- lm(y ~ x, df_train)
```


```{r, solution_03c_b, eval=TRUE}
### quadratic relationship (degree 2)
fit_train_2 <- lm(y ~ x + I(x^2), df_train)
```
...
### cubic relationship (degree 3)
fit_train_3 <- lm(y ~ x + I(x^2) + I(x^3), df_train)
```


```{r, solution_03c_d, eval=TRUE}
### degree 4
fit_train_4 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4), df_train)
```


```{r, solution_03c_e, eval=TRUE}
### degree 5
fit_train_5 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5), df_train)
```


```{r, solution_03c_f, eval=TRUE}
### degree 6
fit_train_6 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6), df_train)
```


```{r, solution_03c_g, eval=TRUE}
### degree 7
fit_train_7 <- lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7), df_train)
```


```{r, solution_03c_h, eval=TRUE}
### degree 8
fit_train_8 <-  lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), df_train)
```


```{r, solution_03c_i, eval=TRUE}
### degree 9
fit_train_9 <-  lm(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8) + I(x^9), df_train)
```
...
```{r, solution_03d}
ho.rmse <- tibble(rmse(fit_train_1, df_holdout), rmse(fit_train_2, df_holdout), 
                  rmse(fit_train_3, df_holdout), rmse(fit_train_4, df_holdout), 
                  rmse(fit_train_5, df_holdout), rmse(fit_train_6, df_holdout), 
                  rmse(fit_train_7, df_holdout), rmse(fit_train_8, df_holdout), 
                  rmse(fit_train_9, df_holdout))
glimpse(ho.rmse)
```
...
According to the hold-out evaluation, the 2nd order polynomial is the best model because it has the lowest RMSE. This is different from the model identified as as best from the training set performance. This model identified as best according to the training set was ranked as the worst according to the hold-out evaluation. 
...
```{r, solution_03f}
coefplot::multiplot(fit_train_9, fit_lm_9)
```

What do you think? I think there is a surprising amount of overlap for each coefficient between the 9th order polynomial model based on the complete training set and the one based on the split training set, especially because the training set in split form did not include the one apparent outlier in the x/y plane of the data set as shown in solution 3b. It is also interesting to note that the higher order terms fit to the complete training set are highly certain compared to those fit to the split training set. There don't appear to be any differences in the statististical significance of coefficients.
...
```{r, solution_03g}
coefplot::multiplot(fit_train_2, fit_lm_2)
```

What do you think? The estimates appear to be quite similar, with means overlapping with confidence intervals for each coefficient between each model. One important difference is that the 1st order term in the model trained on the split training set is not significantly different from 0, whereas the 1st order term in the model trained on the complete training set was evaluated to be significantly greater than 0.
...
I chose R-Squared because it has a simple interpretation of squared correlation of predicted and observed outputs in the holdout set and it takes continuous inputs.
...
### degree 9
set.seed(2001)
mod_lowhigh_9 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + 
...
The model with the highest R-squared is the 6th order model. However, it has a very large confidence interval. Therefore, it makes sense to use the simplest model that's confidence interval overlaps with the 6th order model mean, which happens to be the second order model. It could be argued the 1st order model is best under this reasoning, however, the second order model not only overlaps with the lower end of the 6th order interval, but also the higher end. Under this same reasoning, I would rank the 3rd order model and 4th order model above the 1st order model, giving rankings of 1) 2nd order, 2) 3rd order, 3) 4th order.
...
```{r, solution_04f}
coefplot::multiplot(mod_lowhigh_2$finalModel, mod_lowhigh_3$finalModel, mod_lowhigh_4$finalModel)
```

What do you think?  

All coefficient estimates are similar in the 2nd-, 3rd-, and 4th-order models except for the linear term, which in the quadratic model is positive, more certain, and statistically significantly greater than 0, whereas in the cubic and 4th-order models the linear terms are negative and their confidence intervals overlap with 0. For other parameter estimates, the models have all the same certainty levels, except for the quadratic term, where the 2nd and 3rd order models are more certain than the 4th order model's certainty for the quadratic parameter.
...
The uncertainty in the nonlinear trend is much lower in the low noise dataset than it is in the high noise dataset, while the linear trends for each noise-level appear the same and appear to share similar uncertainty.
...
### degree 7
set.seed(2001)
mod_lowlow_7 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) +
                        I(x^7),
                       method = "lm", 
                       metric = my_metric,
                       trControl = my_ctrl, 
...
### degree 8
set.seed(2001)
mod_lowlow_8 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) +
                        I(x^7) + I(x^8),
                       method = "lm", 
                       metric = my_metric,
                       trControl = my_ctrl, 
...
### degree 9
set.seed(2001)
mod_lowlow_9 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) +
                        I(x^7) + I(x^8) + I(x^9),
                       method = "lm", 
                       metric = my_metric,
                       trControl = my_ctrl, 
...
The best model is the 2nd order model since it's confidence interval overlaps with the model with the best mean R-squared and it is the simplest model that does so.
...
The resampling scheme identified the same model (2nd-order) as the best model and did so for similar reasons as in the high noise case. The low noise case makes clear that the 1st-order model was in fact the poorest model, while this was not entirely clear in the high-noise case given the large confidence intervals for higher-order models. In both cases, though, the 9th order model is identified as a poor model of choice because it is the most complex model while not scoring any better on performance metrics than the second order and lower order models.
...
**Create a scatter plot between the input, `x`, and the response, `y`, for the low noise level data. How does this figure compare to the one you made in Problem 1a)?**  I'm assuming you mean high noise high sample size. 
...
Given high noise is held constant. The slope of the linear trend is more positive in the larger sample size than in the lower sample size. The certainty of both the linear and nonlinear trends appear higher in the larger sample size than the lower sample size.
...
### degree 7
set.seed(2001)
mod_highhigh_7 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) +
                          I(x^7),
                       method = "lm", 
                       metric = my_metric,
                       trControl = my_ctrl, 
...
### degree 8
set.seed(2001)
mod_highhigh_8 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) +
                          I(x^7) + I(x^8),
                       method = "lm", 
                       metric = my_metric,
                       trControl = my_ctrl, 
...
### degree 9
set.seed(2001)
mod_highhigh_9 <- train(y ~ x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) +
                          I(x^7) + I(x^8) + I(x^9),
                       method = "lm", 
                       metric = my_metric,
                       trControl = my_ctrl, 
...
What do you think?  The resampling scheme with a high-noise / high sample size dataset identifies the 2nd order model as the best, which is the same as the model the same resampling scheme identified in the low noise low sample size set and the low sample size high noise set.  The high sample size / high noise case makes clear, similar to the low sample size / low noise case, that the 1st-order model was in fact the poorest model, while this was not entirely clear in the high-noise case given the large confidence intervals for higher-order models. In both cases, though, the 9th order model is identified as a poor model of choice because it is the most complex model while not scoring any better on performance metrics than the second order and lower order models.
...
Noise level negatively correlates with the ability to identify complex behavior and sample size positively correlates with ability to identify complex behavior.
