...
What do you think?  I think the results are the same because the MLE of for the mu parameter of a Bernoulli random variable is the sum of the outcomes (where 1 is assigned to the event and 0 to the non-event) divided by the number of trials. This is same method we used to calculate the batting average.
...
$$ 
\log\left(\mu^x\left(1-\mu\right)^\left(1-x\right)\right)
$$ 
$$
\log\left(\mu^x\right) + \log\left(1-\mu\right)^\left(1-x\right)
$$
$$
x\log\left(\mu\right) + \left(1-x\right)\log\left(1-\mu\right)
$$
...
{
 return (xobs*log(prob) + (1-xobs)*log(1-prob)) 
}
```
...
```{r solution 1f}
log_bernoulli_pmf(x[1], .25)
log_bernoulli_pmf(x[2], .25)
log_bernoulli_pmf(x[3], .25)
log_bernoulli_pmf(x[4], .25)
log_bernoulli_pmf(x[5], .25)
```
...
```{r solution 1g}
log_bernoulli_pmf(x[1:5], .25)
```

The length of the returned result is 5. They are the same values as when the function was called previously, which transformed out of log space is .25, which is accurate given the Bernoulli PMF parametrized with $\mu = 5$.
...
$$ 
\log\left(\prod^N_{n=1}\left(\mu^{x_n}\left(1-\mu\right)^{\left(1-x_n\right)}\right)\right)
$$ 

$$
= \log\left(\sum^N_{n=1}\left(\mu^{x_n}\left(1-\mu\right)^{\left(1-x_n\right)}\right)\right)
$$

$$
= \sum^N_{n=1}\left(\log\left[\mu^{x_n}\left(1-\mu\right)^{\left(1-x_n\right)}\right]\right)
$$
$$
= \sum^N_{n=1}\left(\log\left[\mu^{x_n}\right]\right) + \sum^N_{n=1}\left(\log\left[1-\mu^{\left(1-x_n\right)}\right]\right)
$$
...
$$
= \log\left[\mu\right]\sum^N_{n=1}\left(x_n\right) + \log\left[1-\mu^{}\right]\sum^N_{n=1}\left(1-x_n\right)
$$

$$
= M\log\left[\mu\right] + \left(N-M\right)\log\left[1-\mu^{}\right]
$$
...
for( i in seq_along(log_lik_xa) ){
  log_lik_xa[i] <- m*log(mu_grid[i]) + (n-m)*log(1-mu_grid[i])
}
...
```{r solution_2e}
xa_results %>%
  ggplot(mapping = aes(x = mu, y = log_lik_xa)) +
  geom_line()
```
...
What do you think?  The log-likelihood is a curve with a maximum value at the MLE. The curve flattens near the MLE and drops precipitously in the negative direciton the further it gets away from the MLE.
...
tibble::tibble(
  ...
  geom_line(size = 1.1) + geom_vline(xintercept = mle, size = 1, linetype = 'dashed', color = 'red')
```
...
What do you think? I think the figures look the same aside from the visual details in 3c such as the color of the vertical line.
...
tibble::tibble(
  ...
  mutate(log_lik = dbinom(x = player_hits, size = player_atbats,
                          prob = mu, log = TRUE)) %>%
  ggplot(mapping = aes(x = mu, y = log_lik)) +
  geom_line(size = 1.1) + geom_vline(xintercept = mle, size = 1, linetype = 'dashed', color = 'red')
```
...
What do you think? I think figure 3e is the same as the figures in 3c and 2f in all respects except that the values of log_lik are shifted by a factor equal to the normalizing constant N choose m.
...
In the uniform set, the prior number of trials is 2. In the informed set, the prior number of trials is 40.
...
For the uniform set, the prior expected value for the event probability is 1/2. For the informed set, the prior expected value for the event probability is 1/4.
...
What do you think?  The majority of the probability density of the informed prior is concentrated around mu = .25, while the probability density of the uniform prior is the same for all possible values of mu (assigning all possible values of mu an equal probability). The probability density of the informed prior 'rules out' values of mu close to 0 and values of mu greater than .5 in the sense that it assigns an extremely small probability to those value or a 0 probability to those values. The probability density of the uninformed prior does not rule out any of the possible values of mu. 
...
Add the log-likelihood from 5a to the log-prior from 5b
...
$$
Multiply log($\mu$) and log(1-$\mu$) by the variables in parentheses

$$
m\log\left(\mu\right) + N\log\left(1 - \mu\right)-m\log\left(1 - \mu\right) + a\log\left(\mu\right) - 1\log\left(\mu\right) + b\log\left(1 - \mu\right)-1\log\left(1 - \mu\right)
$$
Factor out log($\mu$) and log(1-$\mu$)

$$
\left (m+a-1 \right ) \log\left(\mu\right) + \left(N-m+b-1\right)\log\left(1 - \mu\right)
$$
This gives the unnormalized, updated/posterior beta distribution, where the new a is equal to a+m and the new b is equal to N - m + b
...
$$ 
\frac{d}{d\mu}\left( \left (m+a-1 \right ) \log\left(\mu\right) \right) + \frac{d}{d\mu}\left( \left(N-m+b-1\right)\log\left(1 - \mu\right) \right )
$$

$$ 
= \frac{m+a-1}{\mu} - \frac{N-m+b-1}{1 - \mu}
$$

$$ 
= \frac{a_{new}-1}{\mu} - \frac{b_{new}-1}{1 - \mu}
$$
...
$$
\frac{a_{new}-1}{\mu_{MAP}} - \frac{b_{new}-1}{1 - \mu_{MAP}} = 0
$$

$$
\frac{a_{new}-1}{\mu_{MAP}} = \frac{b_{new}-1}{1 - \mu_{MAP}}
$$

$$
\frac{1 - \mu_{MAP}}{\mu_{MAP}} = \frac{b_{new}-1}{a_{new}-1}
$$

$$
\frac{1}{\mu_{MAP}} - 1 = \frac{b_{new}-1}{a_{new}-1}
$$

$$
\frac{1}{\mu_{MAP}} = \frac{b_{new}-1 + a_{new}-1}{a_{new}-1}
$$

$$
\frac{1}{\mu_{MAP}} = \frac{b_{new}+a_{new}-2}{a_{new}-1}
$$

$$
\mu_{MAP} = \frac{a_{new}-1}{b_{new}+a_{new}-2}
$$
...
paste0("Updated a from a_uniform: ", new_a_uniform)
paste0("Updated b from b_uniform: ", new_b_uniform)
paste0("Updated a from a_inform: ", new_a_inform)
paste0("Updated b from b_inform: ", new_b_inform)
```
...
```{r solution_6b}
posterior_beta <- tibble(
  prior_type = c("uniform", "informed"),
  mean = c(new_a_uniform/(new_a_uniform + new_b_uniform),
           new_a_inform/(new_a_inform + new_b_inform)),
  mode = c((new_a_uniform -1)/(new_b_uniform + new_a_uniform - 2),
           (new_a_inform - 1)/(new_b_inform + new_a_inform -2)),
  quantile.05 = c(qbeta(.05, shape1 = new_a_uniform, shape2 = new_b_uniform),
                  qbeta(.05, shape1 = new_a_inform, shape2 = new_b_inform)),
  quantile.95 = c(qbeta(.95, shape1 = new_a_uniform, shape2 = new_b_uniform),
                  qbeta(.95, shape1 = new_a_inform, shape2 = new_b_inform))
)
posterior_beta
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, shape1 = new_a_uniform, shape2 = new_b_uniform)) %>%
  ggplot(mapping = aes(x = mu, y = beta_pdf)) +
  geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, shape1 = new_a_inform, shape2 = new_b_inform)) %>%
  ggplot(mapping = aes(x = mu, y = beta_pdf)) +
  geom_line()
```
...
What do you think?  The posterior belief based on the informed prior is more precise than the posterior belief based on the uniform prior. The posterior belief based on the informed prior is also centered closer to the mode of the informed prior (0.25) than the posterior belief based on the uniform prior. Therefore, the posterior belief based on the informed prior "rules out" values approximately less than .125 and approximately greater than .5 while the posterior belief based on the uniform prior similarly "rules out" values approximately less than .125 but differently only "rules out" possible values of mu approximately greater than .75. That is, the posterior belief based on the uniform prior "allows" higher values of mu than the posterior belief based on the informed prior.
...
```{r solution_7a}
new_a_uniform <- a_uniform + season_hits
new_b_uniform <- b_uniform + season_atbats - season_hits
new_a_inform <- a_inform + season_hits
new_b_inform <- b_inform + season_atbats - season_hits
paste0("Updated a from a_uniform: ", new_a_uniform)
paste0("Updated b from b_uniform: ", new_b_uniform)
paste0("Updated a from a_inform: ", new_a_inform)
paste0("Updated b from b_inform: ", new_b_inform)
...
What do you think?  I think the influence of the priors on the posteriors is greater when the sample size is small compared to when the sample size is large. This is illustrated by the fact that the posteriors in the large sample size case for both the uniform prior and the informed prior are virtually the same, while there were notable differences between the posteriors for the uniform prior and the informed prior in the small sample size case.
