...
$$
\mu_{0} \pm 2 \tau_{0} = \left[115, 135\right]
$$
Since this a normal (symmetric) distribution, we can calculate the mean is in the center of this (approximately) 95% uncertainty interval, that is,

$$
\mu_0 = 125
$$
Since the standard deviation is a constant, $\tau_0$, we can calculate it by taking the absolute difference between the mean and either end of the (approximate) 95% uncertainty interval and dividing by 2.

$$
\tau_0 = \frac{\left|125 - 115\right|}{2} = 5
$$
Therefore, the Gaussian prior is (approximately) distributed as

$$
p\left( \mu \mid \mu_0, \tau_0 \right) \sim \mathcal{N}\left(125, 5\right)
$$
...
The interval between these two quantiles contains 50% of the probability density. 

```{r solution_1b}
paste0("The 25th quantile is: ", qnorm(.25, 125, 5))
paste0("The 75th quantile is: ", qnorm(.75, 125, 5))
```
...
$$
z = \frac{x - \mu_0}{\tau_0} = \frac{117 - 125}{5} = -1.6
$$

According to z-score tables (standard normal tables), 1.6 standard deviations below the mean is the 5.48th percentile. Therefore 5.48% of parts will have performance values of 117 units or less.
...
As given in lecture, "the posterior standard deviation is defined in terms of the posterior precision:"
...
$$
\tau_{N}^2 = \frac{1}{\frac{1}{\tau_{0}^2} + \frac{N}{\sigma^2}} = \frac{1}{\frac{\sigma^2 + N\tau_{0}^2}{\tau_{0}^2\sigma^2}} = \frac{\tau_{0}^2\sigma^2}{\sigma^2 + N\tau_{0}^2}
$$

and take the square root to get the posterior standard deviation given $\sigma = 5$ and the Gaussian prior variance given in Problem 1a).

$$
\tau_{N} = \frac{\tau_{0}\sigma}{\sqrt{\sigma^2 + N\tau_{0}^2}}\\
\sqrt{\sigma^2 + N\tau_{0}^2} = \frac{\tau_{0}\sigma}{\tau_{N}}\\
\sigma^2 + N\tau_{0}^2 = \left(\frac{\tau_{0}\sigma}{\tau_{N}}\right)^2\\
N = \frac{\left(\frac{\tau_{0}\sigma}{\tau_{N}}\right)^2 - \sigma^2}{\tau_{0}^2}
$$

We can determine required sample sizes, N, by plugging in known values. For $\tau_N = 2$

$$
N = \frac{156.25 - 25}{25} = 5.25 \approx 6 \text{ observations}
$$

For $\tau_N = 0.5$

$$
N = \frac{2500 - 25}{25} = 99 \text{ observations}
$$
...
As given in lecture, "in the limit of an infinitely diffuse prior, the posterior [distribution] converges to:"

$$
p\left(\mu\mid x, \sigma\right) \rightarrow \text{normal}(\bar{x},\frac{\sigma}{\sqrt{N}})
$$

and so $\tau_N$ converges to:

$$
\tau_N \rightarrow \frac{\sigma}{\sqrt{N}}
$$

Setting the left and right hand sides of this convergence equal to each other:

$$
\tau_N = \frac{\sigma}{\sqrt{N}}\\
N = \frac{\sigma^2}{\tau_N^2}
$$

Setting $\tau_N =3$ and $\sigma = 15$

$$
N = \frac{\sigma^2}{\tau_N^2} = 225/9 = 25 \text{ observations}
$$
...
$$
\log\left(p\left(\mu \mid \mathbf{x},\sigma\right)\right) \propto \log\left(\prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \times \mathrm{normal}\left( \mu \mid \mu_0, \tau_0\right)\right) = 
$$

$$
\sum_{n=1}^{N} \left[ \log \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \right] + \log \left( \mathrm{normal} \left( \mu \mid \mu_0, \tau_0 \right) \right) = 
$$

$$
\sum_{n=1}^{N} \log \left[ \frac{1}{\sqrt{2 \pi \sigma^2}} \exp \left( - \frac{1}{2\sigma^2} \left( x_n - \mu \right)^2 \right) \right] + \log \left[ \frac{1}{\sqrt{2 \pi \tau_0^2}} \exp{ \left( -\frac{1}{2\tau_0^2} \left( \mu - \mu_{0} \right)^2 \right)} \right] = 
$$

$$
\sum_{n=1}^{N} \left[\log(1)-\log\left(\sqrt{2\pi\sigma^2}\right) + \log\left[\exp\left(-\frac{1}{2\sigma^2}\left(x_n-\mu\right)^2\right)\right]\right] + \log(1)-\log\left(\sqrt{2\pi\tau_0^2}\right) + \log\left[\exp\left(-\frac{1}{2\tau_0^2}\left(\mu-\mu_0\right)^2\right)\right] = 
$$

$$
\left( \text{by dropping terms not containing } \mu \text{ that drop out when taking the derivative of } \mu \right) 
$$
...
$$
\frac{N}{\sigma^2}\left(\bar{x} - \mu_{MAP}\right) - \frac{1}{\tau_0^2}\left(\mu_{MAP}-\mu_0\right) = 0 \\
N\tau_0^2\left(\bar{x} - \mu_{MAP}\right) - \sigma^2\left(\mu_{MAP}-\mu_0\right) = 0 \\
N\tau_0^2\bar{x} - N\tau_0^2\mu_{MAP} - \sigma^2\mu_{MAP}+\sigma^2\mu_0 = 0 \\
- N\tau_0^2\mu_{MAP} - \sigma^2\mu_{MAP} = -\sigma^2\mu_0 - N\tau_0^2\bar{x} \\
-\mu_{MAP}\left(\sigma^2 + N\tau_0^2\right) =- \sigma^2\mu_0 - N\tau_0^2\bar{x} \\
...
The posterior mode expression is equivalent to that for the posterior mean given in lecture because the posterior distribution is normal, which is a symmetric distribution, which means the mean = mode = median. The equivalence of the posterior mode expression and the posterior mean expression is demonstrated in the code chunk below when choosing random values for the variables:
...
paste0("Posterior mode: ", (sigma2*mu0 + N*tau02*xbar)/(sigma2 + N*tau02))
paste0("Posterior mean: ", ((1/tau02)*mu0 + (N/sigma2)*xbar)/(1/tau02 + N/sigma2))
```
...
$$
\log\left(p\left(\mu \mid \mathbf{x},\sigma\right)\right) \propto \log\left(\prod_{n=1}^{N} \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right) \times \mathrm{Exp}\left( \sigma \mid \lambda\right)\right) = \\
\sum_{n=1}^{N}\left[\log \left( \mathrm{normal} \left( x_n \mid \mu, \sigma \right) \right)\right] + \log\left(\mathrm{Exp}\left( \sigma \mid \lambda\right)\right)= \\
\sum_{n=1}^{N}\left[\log(1)-\log\left(\sqrt{2\pi\sigma^2}\right) + \log\left[\exp\left(-\frac{1}{2\sigma^2}\left(x_n-\mu\right)^2\right)\right]\right] + \log(\lambda) - \lambda\sigma = \\
\sum_{n=1}^{N}\left[-\frac{1}{2}\log\left(2\pi\right) - \frac{1}{2}\log(\sigma^2) -\frac{1}{2\sigma^2}\left(x_n-\mu\right)^2\right] - \lambda\sigma = \\
\sum_{n=1}^{N}\left[- \log(\sigma) -\frac{1}{2\sigma^2}\left(x_n-\mu\right)^2\right] - \lambda\sigma 
$$
...
$$
\sum_{n=1}^{N}\left[- \log(\sigma) -\frac{1}{2\sigma^2}\left(x_n-\mu\right)^2\right] - \lambda\sigma=\\
-N\log(\sigma) - \frac{Nv}{2\sigma^2} - \lambda\sigma
$$

The quantity v is referred to as sufficient because it contains enough information to sufficiently describe all our observations. That is, our observations entail no new information given the sufficient statistics $v$. Therefore, after making use of v this new expression does not depend on individual observations.
...
{
  -N*log(sigma) - N*v/(2*sigma^2) - lambda*sigma
}
```
...
The log-posterior appears to have just one mode. It is parabolic in the sense that it seems approximately U-shaped with very wide-angled curves.
...
The inverse link function is the exponential. 

$$
\varphi = g\left(\sigma\right) = \log \left(\sigma\right)\\
\sigma = g^{-1}(\varphi) = \exp(\varphi)\\
\frac{d}{d\varphi}\exp(\varphi) = \exp(\varphi)\\
\log\left(\exp(\varphi)\right) = \varphi
$$
...
tibble::tibble(
  varphi = seq(from = 0.5, to = 4.5, length.out = 101)
) %>% 
...
The unnormalized log posterior on the transformed variable looks more like half of a downward facing parabola than the unnormalized log-posterior on the untransformed variable.
...
fnscale tells optim() to maximize if fnscale is negative and tells optim() to minimize if fnscale is positive. gr should be set to NULL
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x = x, 
                       y = my_func(x, my_constants))) +
  geom_line() + geom_vline(xintercept = opt_result$par, color = 'red')
```
...
The resulting posterior mode is the same
...
tibble::tibble(
  ...
  mutate(log_post = log_post_varphi_unnorm(varphi, N, v, 0.75)) %>%
  ggplot(mapping = aes(x = varphi, y = log_post)) +
  geom_line() + geom_vline(xintercept = varphi_opt_a$par, color = 'red')
```
