...
```{r, solution_01a}
paste0("The sample average is: ", mean(hw06_df$x))
paste0("The sample standard deviation is: ", sd(hw06_df$x))
paste0("The sample minimum is: ", min(hw06_df$x))
paste0("The sample maximum is: ", mean(hw06_df$x))
```
...
The two ends of the approximate 95% prior uncertainty intervals are equidistant from the mean, which means we can take the average of these two numbers to get the prior median, which is equal to the prior mean.

$$
\frac{10 + 12}{2} = 11 = \text{prior median} = \text{prior mean} = \mu_0
$$

Since 10 and 12 form the two ends of this approximately 95% prior uncertainty interval, this means 10 and 12 are approximately 2 standard deviations away from our derived prior mean. We can calculate $\tau_0$ by taking the absolute difference between the mean and either end of the (approximate) 95% uncertainty interval and dividing by 2.

$$
\tau_0 = \frac{\left|11 - 12\right|}{2} = 0.5
$$
...
sigma_grid_lwr <- 1
sigma_grid_upr <- qexp(.99, rate = hw06_info$sigma_rate)
```
...
The sample average is approximately 0.7 units less than posterior mean given by the joint posterior mode. The sample standard deviation is approximately 0.5 units greater than the posterior standard deviation given by the joint posterior mode. As such, the sample standard deviation appears more similar to its corresponding value in the joint posterior mode than the sample average does to its corresponding value in the joint posterior mode. Regardless, each are similar to the joint posterior mode. Based on the visualization, it can be stated the most likely values of mu are concentrated around roughly 10.8, while the most likely values of sigma are concentrated around roughly 2.6. As the posterior standard deviation becomes larger, the spread of possible values of mu becomes smaller. As the posterior mean becomes closer to ~10.8, the spread of possible values of sigma starts to exclude larger values such as sigma greater than 7.
...
$$
\sum_{n=1}^{N} \left[-\frac{1}{2\sigma^2}\left(x_n-\mu\right)^2\right] - \frac{1}{2\tau_0^2}\left(\mu-\mu_0\right)^2 
$$

And it's first derivative with respect to $\mu$:

$$
\frac{d}{d\mu}\left(\sum_{n=1}^{N}-\frac{1}{2\sigma^2}\left(x_n-\mu\right)^2 - \frac{1}{2\tau_0^2}\left(\mu-\mu_0\right)^2\right) = \\
\sum_{n=1}^{N}\frac{d}{d\mu}\left(-\frac{1}{2\sigma^2}\left(x_n-\mu\right)^2\right) - \frac{d}{d\mu}\left(\frac{1}{2\tau_0^2}\left(\mu-\mu_0\right)^2\right) = \\
\sum_{n=1}^{N}\left[\frac{1}{\sigma^2}\left(x_n-\mu\right)\right] - \frac{d}{d\mu}\left(\frac{1}{2\tau_0^2}\left(\mu-\mu_0\right)^2\right) = \\
\frac{1}{\sigma^2}\sum_{n=1}^{N}\left(x_n-\mu\right) - \frac{1}{\tau_0^2}\left(\mu-\mu_0\right) = \\
\frac{N}{\sigma^2}\left(\bar{x} - \mu\right) - \frac{1}{\tau_0^2}\left(\mu-\mu_0\right)
$$
And so the second derivative is:

$$
\frac{d}{d\mu}\left[\frac{N}{\sigma^2}\bar{x} - \frac{N}{\sigma^2}\mu - \frac{1}{\tau_0^2}\mu+\frac{1}{\tau_0^2}\mu_0 \right] = \\
0-\frac{N}{\sigma^2} - \frac{1}{\tau_0^2} + 0 = \\
...
The final expression of the second derivative $-\frac{N}{\sigma^2} - \frac{1}{\tau_0^2}$ is always negative because $N$, $\sigma$, and $\tau_0$ are all always positive numbers. Therefore, the second derivative is always negative. This tells me that when I determined the expression for the posterior mode (by setting the first derivative with respect to $\mu$ equal to 0 and solving for $\mu_{map}$), that this is the only time the first derivative can be equal to zero, because it is always decreasing according to the second derivative. Due to the fact that it is always decreasing, this also tells me that the mode derived from setting the first derivative equal to zero must be a maximum. That is, the first derivative is initially positive as it approaches the mode, then decreases until it is 0, indicating a peak of the log-posterior, and then the first derivative becomes increasingly negative as the value of $\mu$ moves past the mode, meaning the slope of the log-posterior is negative. Another way to consider this is that the second derivative being negative means negative curvature, such that the first derivative equaling zero once and only once must be a maximum.
...
The approximate posterior mean is equal to $\mu_{map}$ identified in the last assignment.

$$
\mu_{MAP} = \frac{\sigma^2\mu_0 + N\tau_0^2\bar{x}}{\sigma^2 + N\tau_0^2}
$$

The approximate posterior standard deviation is the inverse of the negative Hessian matrix, which in this case is just the second derivative of the un-normalized log posterior with respect to $\mu$ times $-1$ raised to the power of $-1$. $\sigma$ was given in the problem statement as $3$, $N = 8$, $\tau_0 = 0.5$

$$
\frac{d^2}{d\mu^2}\left(\sum_{n=1}^{N}-\frac{1}{2\sigma^2}\left(x_n-\mu\right)^2 - \frac{1}{2\tau_0^2}\left(\mu-\mu_0\right)^2\right) =-\frac{N}{\sigma^2} - \frac{1}{\tau_0^2} = \\
-\frac{8}{9} - \frac{1}{0.5^2} = -\frac{8}{9} - 4 = -\frac{44}{9}
$$
We take the negative of this result and invert it

$$
\left[-\left(-\frac{44}{9}\right)\right]^{-1} = \left(\frac{44}{9}\right)^{-1} = \frac{9}{44} = \mathrm{s}_N^2 \\
\mathrm{s}_N = \sqrt{\frac{9}{44}} = \frac{3}{6.63324958} = 0.452267017
$$
...
The Laplace approximation is a multivariate normal distribution. 

The posterior mean of the Laplace approximation is equal to the exact posterior mean by definition. We center the Laplace distribution around the log-posterior mode of the exact distribution, which is equal to the log-posterior mean because the log-posterior in this normal-normal model is normally distributed.

The Laplace approximation approximates the exact posterior standard deviation by taking the inverse of the negative Hessian matrix. In this specific case, the negative Hessian matrix is equal to the sum of the data precision and prior precision, which was given in lecture to be equal to the exact posterior precision. That is,

$$
\frac{N}{\sigma^2} + \frac{1}{\tau_0^2} = \frac{1}{\mathrm{s}_N^2} = \frac{1}{\tau_N^2}
$$
If we invert this expression and take the square root, we obtain the Laplace posterior standard deviation as well as the exact posterior standard deviation.
...
It is useful to use this transformation because the log-space forbids approximations of $\sigma$ to be $<= 0$, which a normal distribution by definition allows. 
...
$$
p \left( \mu, \sigma \mid \mathbf{x} \right) \propto \prod_{n=1}^{N} \left( \mathrm{normal} \left(x_n \mid \mu, g^{-1}\left(\varphi \right) \right) \right) \times \mathrm{normal}\left(\mu \mid \mu_0, \tau_0\right) \times \mathrm{Exp}\left(g^{-1}\left(\varphi \right) \mid \lambda=0.5\right) \cdot \left| \frac{d}{d \varphi} \left(g^{-1}\left(\varphi \right)\right) \right|
$$
...
$$
\sigma = g^{-1}\left(\varphi \right) = \exp \left( \varphi \right)
$$

Write the log of the derivative adjustment in an equation block.  

$$
\left| \frac{d}{d \varphi} \left(g^{-1}\left(\varphi \right)\right) \right| = \exp \left( \varphi \right) \\
\log \left( \exp \left( \varphi \right) \right) = \varphi
$$
...
my_cv_logpost <- function(unknowns, my_info)
{
  ...
  log_deriv_adjust <- log(abs(exp(lik_varphi))) 
...
Describe the contour shapes of the log-posterior and how the overall shape compares to the log-posterior in the original parameter space between $\mu$ and $\sigma$. 

The contour shapes appear like concentric circles and the overall shape of the log-posterior is less "stretched" on the $\varphi$-axis than the log-posterior in the original parameter space between $\mu$ and $\sigma$ on the $\sigma$ axis. The posterior mode for $\mu$ given by the surface appears the same.
...
The identified optimal values are same and the Hessian matrices are the same. That the identified optimal values would be the same was hinted at by my visualization of the log-posterior surface because the visualization reveals that the surface is unimodal, meaning all paths "travel" to it no matter where one starts on the surface. The only difference between the two optim results is the number of calls optim made to the function and (lack of) gradient to compute the finite-difference approximation to the gradient and to compute the Hessian matrix.
...
var_matrix <- laplace_result$var_matrix
post_corr <- var_matrix[2,1]/(sqrt(var_matrix[1,1])*sqrt(var_matrix[2,2]))
post_corr
```

The posterior correlation coefficient is 0.1211003. It was calculated by dividing the covariance between $\mu$ and $\varphi$ by the product of the standard deviations (square roots of variances) of $\mu$ and $\varphi$.
...
```{r solution_5a}
z <- (mean(hw06_info$xobs) - laplace_result$mode[1])/laplace_result$mode[2]
z
```
The z-score for the sample average under the laplace approximated MVN is -0.5847328, which based on a standard Z table, tells us there is an approximately .28 probability that $\mu$ is less than the sample average.
...
The inverse transformation function applied to the posterior mean on varphi does not equal the posterior mean on sigma from the random sampling procedure.
...
The posterior distribution on $\sigma$ does not look Gaussian. It is right-skewed / asymmetric.
...
```{r solution_5e}
post_prob_sigma <- sum(post_samples$sigma > 4)/length(post_samples$sigma)
post_prob_sigma
```
I estimate the probability that the noise is greater than 4 units by counting the number of inverse-transformed varphi observations from the random sampling from mvrnorm that are greater than 4 units and dividing by the total number of inverse-transformed varphi observations from the random sampling procedure. My estimated posterior probability that $\sigma$ is greater than 4 is 0.1084
