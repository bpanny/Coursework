...
Xmat_quad_small
```
The values contained in the first column are 1. This stands for our "fake" input variable $x_{0}$ corresponding to an identity multiplier to our intercept coefficient, allowing us to write our linear model easily in matrix notation.
...
covmat_quad_small_noprior <- 1^2 * solve( t(Xmat_quad_small[,,drop=FALSE]) %*% Xmat_quad_small[,,drop=FALSE] )
```


```{r, solution_for_1d_b, eval=TRUE}
### posterior covariance matrix for the BIG data
covmat_quad_big_noprior <- 1^2 * solve( t(Xmat_quad_big[,,drop=FALSE]) %*% Xmat_quad_big[,,drop=FALSE] )
```
...
Each covariance matrix has three rows and three columns.
...
```{r, solution_1f}
paste0("The posterior standard deviation on the quadratic feature for the small data case and no prior: ", sqrt(covmat_quad_small_noprior[3,3]))
paste0("The posterior standard deviation on the quadratic feature for the big data case and no prior: ", sqrt(covmat_quad_big_noprior[3,3]))
```
...
The values displayed would not change if sigma was assumed equal to five compared to sigma equal to one or any other valid number for sigma, The posterior means do not depend on sigma.
...
```{r, solution_1h}
paste0("The posterior 95% uncertainty interval on the quadratic feature in the small data case is [", qnorm(0.025, mean = postmeans_quad_small_noprior[3,1], sd = sqrt(covmat_quad_small_noprior[3,3])), ", ", qnorm(0.975, mean = postmeans_quad_small_noprior[3,1], sd = sqrt(covmat_quad_small_noprior[3,3])), "]")

paste0("The posterior 95% uncertainty interval on the quadratic feature in the big data case is [", qnorm(0.025, mean = postmeans_quad_big_noprior[3,1], sd = sqrt(covmat_quad_big_noprior[3,3])), ", ", qnorm(0.975, mean = postmeans_quad_big_noprior[3,1], sd = sqrt(covmat_quad_big_noprior[3,3])), "]")
```

Assuming $\sigma=1$, the quadratic feature is considered statistically significantly different from 0 in the small data case as well as the big data case. In the small data case, the quadratic feature is statistically significantly greater than 0. In the big data case, the quadratic feature is statistically significantly less than 0.
...
B0_inform <- matrix(c(1.4^2, 0, 0, 0, 1.4^2, 0, 0, 0, 1.4^2), nrow = 3, ncol = 3)
B0_inform
```
...
B0_vague <- matrix(c(30^2, 0, 0, 0, 30^2, 0, 0, 0, 30^2), nrow = 3, ncol = 3)
B0_vague
```
...
covmat_quad_small_inform <- solve(solve(B0_inform) + (t(Xmat_quad_small[,,drop=FALSE]) %*% Xmat_quad_small[,,drop=FALSE]) / (1^2))
...
covmat_quad_big_inform <- solve(solve(B0_inform) + (t(Xmat_quad_big[,,drop=FALSE]) %*% Xmat_quad_big[,,drop=FALSE]) / (1^2))
...
covmat_quad_small_vague <- solve(solve(B0_vague) + (t(Xmat_quad_small[,,drop=FALSE]) %*% Xmat_quad_small[,,drop=FALSE]) / (1^2))
...
covmat_quad_big_vague <- solve(solve(B0_vague) + (t(Xmat_quad_big[,,drop=FALSE]) %*% Xmat_quad_big[,,drop=FALSE]) / (1^2))
...
```{r solution_2d}
paste0("The posterior standard deviation on the quadratic feature for the small data case with an informative prior: ", sqrt(covmat_quad_small_inform[3,3]))
paste0("The posterior standard deviation on the quadratic feature for the big data case with an informative prior: ", sqrt(covmat_quad_big_inform[3,3]))
```

In the big data case, the posterior standard deviation with an informative prior is similar to the posterior standard deviation with no prior. In the small data case, the standard deviations with an informative prior vs. no prior are different by about .4 units, where the informative prior has a smaller posterior standard deviation
...
```{r, solution_2e}
paste0("The posterior standard deviation on the quadratic feature for the small data case with a vague prior: ", sqrt(covmat_quad_small_vague[3,3]))
paste0("The posterior standard deviation on the quadratic feature for the big data case with a vague prior: ", sqrt(covmat_quad_big_vague[3,3]))

paste0("The posterior standard deviation on the quadratic feature for the small data case and no prior: ", sqrt(covmat_quad_small_noprior[3,3]))
paste0("The posterior standard deviation on the quadratic feature for the big data case and no prior: ", sqrt(covmat_quad_big_noprior[3,3]))
```
The posterior standard deviations on the quadratic feature for both the small and big data cases with a vague prior are very similar to the posterior standard deviations on the quadratic feature for both the small and big data cases with no prior.
...
The informative prior constrains the posterior standard deviations and covariances of the unknowns when the dataset is small compared to no prior. Regardless of prior, covariances and posterior standard deviations come out similarly when the dataset is big; a vague prior produces similar results to no prior.
...
```{r, solution_03c_c}
paste0("Posterior Modes: ", laplace_quad$mode)
paste0("Posterior Standard Deviations: ", sqrt(diag(laplace_quad$var_matrix)))
```

Should you be worried about the initial guess impacted results?

No, because the laplace approximation is an MVN with a singular mode where all parameter estimates are at their most likely values.
...
set.seed(87123)
post_samples_quad <- generate_lm_post_samples(laplace_quad, length(laplace_quad$mode) - 1, 2500)
```

Create the posterior histogram on $\beta_2$ and calculate the probability that $\beta_2$ is greater than zero.  

```{r, solution_03d_c}
paste0("Probability that Beta2 is greater than 0: ", mean(post_samples_quad$beta_02 > 0))
post_samples_quad %>%
  ggplot(mapping = aes(beta_02)) +
...
The 30 rows correspond to the number of observations in the training set, the 2500 columns correspond to the number of samples.
...
The length of the error_quad_post01 vector is 30 error terms, one for each observation/output pair.
...
I am averaging over the absolute errors between the posterior mean trend samples and the training y observations. The three MAE values are not the same. They are different because the posterior mean trend samples are *random* samples from the MVN laplace distribution for the beta coefficients, so the variation in the posterior beta coefficients produces variations in the MAE metric.
...
absError_quad_mat <- abs(as.vector(y_quad_big_col) - post_trend_samples_quad)
### dimensions?
...
Why is the MAE uncertain?  The MAE is uncertain because its calculation relies on the difference between y observations and the posterior mean trends at particular x input values. The posterior mean trends rely on beta coefficients that have to be estimated. In our formulation, we have a posterior distribution over beta coefficients and sampling from this posterior distribution gives us many particular combinations of beta coefficients to use to estimate the mean trend. Therefore, we calculate many samples of the MAE, which gives us a distribution of MAEs that corresponds to our input data, output data, and our posterior distribution over beta coefficients.
...
train_df %>% 
  ggplot(mapping = aes(x = x, y = y)) +
  geom_point() +
  geom_point(mapping = aes(x = x, y = y), data = test_df, color = 'red')
```
...
There are 150 rows (corresponding to the number of training observations) in the training design matrix. There are 50 rows (corresponding to the number of test observations) in the test matrix
...
manage_spline_fit <- function(Xtrain, logpost_func, my_settings)
{
  ...
  init_beta <- rnorm(n = ncol(my_settings$design_matrix), mean = 0 , sd = 1)
...
all_spline_mae_results %>%
  ...
  stat_summary(mapping = aes(color = dataset), geom = 'line', fun = 'median') + 
...
all_spline_medians <- all_spline_mae_results %>% group_by(J, dataset)
all_spline_medians %>% summarise(median(mae))
```

Which degrees-of-freedom overfit? 

From the visualization and differences between summarized (median) MAEs, it appears as if models with approximately 9 degrees of freedom and upwards are overfit. This is because models with greater than 7 degrees of freedom have about the same MAE distribution on the test set as the model with 7 degrees of freedom. That is, models with 9 or higher degrees of freedom are increasing in complexity without improving accuracy.
...
The best model is model with J = 7. Model with J = 9 is a close second. Model with J = 11 is a distant third. 
...
Assessments from the single_train/test split appear misleading because for many of the higher DOF models there are low MAEs for test set predictions, even though the models get very complex without improving MAE measured accuracy. Insofar as we take the simplest model from the train/test split to be the best model, the single train/test split is equivalent to the evidence-based assessment. However, this requires believing in the rule that one should pick the model with the best test set performance that minimizes complexity while maintaining accuracy. Otherwise, we might say the assessments from the single train/test split suggests all models with DOF >= 7 are equivalent of well-fit. The evidence-based assessments rule this out automatically because inherent to the evidence-based assessment is a complexity penalty.
