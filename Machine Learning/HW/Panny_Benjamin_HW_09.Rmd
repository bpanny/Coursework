...
The scales of the inputs seem similar. They are virtually all within -4 and 4 and the IQRs are nearly all bounded by -3 and 3.
...
x03 and x07 seem related to the output, the rest of the inputs seem too noisy to visually glean a relationship to the output.
...
```{r solution_1f}
broom::tidy(modA) %>% filter(p.value < .05)
```

Non-Bayesians would identify the coefficients for the intercept, x03, x07, x16, x21, x33 and x67 as statistically significant.
...
loss_ridge <- function(betas, my_info)
{
  ...
  mu <- my_info$design_matrix %*% betas
  
  # calculate MSE
  MSE <- 1/(nrow(mu)) * t(my_info$yobs - mu) %*% (my_info$yobs - mu)
  
  # calculate ridge penalty
  penalty <- sum(betas^2)
  
  # return effective total loss
  0.5 * MSE + my_info$lambda * penalty
}
```
...
```{r, solution_02d_2}
check_ridge_fit_low_penalty <- fit_regularized_regression(.001, loss_ridge, reg_info)
check_ridge_fit_low_penalty %>% glimpse()
```
...
```{r solution_2e}
check_ridge_fit_lowpenalty <- fit_regularized_regression(.001, loss_ridge, reg_info)
viz_compare_to_mles(check_ridge_fit_lowpenalty, modA)
```

How do the coefficients compare?  

The coefficients appear to be virtually the same.
...
The high-penalty coefficients hover at or near 0, while the majority of MLE coefficients are at least a few tenths away from 0.
...
The coefficient paths converge to 0 as the regularization strength increases, whereas they start relatively spread out between ~-2.5 and 2 when lambda is small (the regularization strength is low).
...
They the low penalty lasso coefficients appear virtually the same as the MLE coefficients.
...
The high-penalty lasso coefficients hover at or very close to 0, while the majority of MLE coefficients are at least a few tenths away from 0.
...
The lasso coefficient paths are similar to the ridge coefficient paths in that they converge to 0 as the regularization strength increases, whereas they start relatively spread out between ~-2.5 and 2 when lambda is small (the regularization strength is low). The difference between the lasso coefficients and ridge coefficients is that the lasso coefficients seem to converge to 0 more quickly than the ridge coefficients. This is particularly notable around 0 < log(lambda) < 2. The ridge coefficients seem to converge on a small range around zero whereas the lasso coefficients appear to become exactly zero  when log(lambda) get close to and as it passes 0.
...
train_and_test_regularized <- function(lambda_use, train_data, test_data, loss_func)
{
  ...
  # add the regularization strength to the list of required information
  info_train$lambda <- lambda_use 
  
  # train the model
  train_results <- optim( start_guess,
                loss_func,
                gr = NULL,
                info_train,
                method = "BFGS",
  ...
  # regularized beta estimates
  train_results_tbl <- tibble::tibble(
    term = colnames(info_train$design_matrix),
    estimate = train_results$par
  ) %>% 
    mutate(lambda = info_train$lambda)
  
  # extract the training set coefficient estimates (completed for you)
  beta_estimates <- train_results_tbl %>% pull(estimate)
  ...
  MSE_test <- 1/(nrow(mu_test)) * t(test_data$y - mu_test) %*% (test_data$y - mu_test) 
...
```{r solution_4bb}
train_test_lasso_results %>% arrange(MSE_test)
```

Which $\lambda$ is the best?  

The best lambda according to the MSE score on the test set is $2.511886 \times 10^{-1}$.
...
```{r solution_4c}
train_test_lasso_result_asc_mse_test <- train_test_lasso_results %>% arrange(MSE_test)
lasso_path_results_low_lambda <- lasso_path_results %>% filter(lambda == train_test_lasso_result_asc_mse_test$lambda[1])
viz_compare_to_mles(lasso_path_results_low_lambda, modA)

```

The cv.glmnet estimates contain more coefficients that hover at or near zero compared to the MLE coefficients. However, some of the cv.glmnet estimates move away from 0, apparently always in the direction of the MLEs, with various levels of closeness to the MLE coefficients.
...
You need to create the design matrix consistent with the `glmnet` requirements before fitting the `glmnet` model. You must therefore create a new design matri for the linear additive features for all inputs but **remove** the intercept column from the design matrix.  
...
The glmnet paths appear the same as the lasso paths from problem 3d.
...
```{r, solution_04f_1}
set.seed(71231)
lasso_cv <- cv.glmnet(Xenet, yenet, lambda = lambda_grid, nfolds = 5)
plot(lasso_cv)
```
...
The number of non-zero features according to the best model is 6 and the number of non-zero features according to the model suggested by the 1 standard error rule is 5.
...
```{r solution_4g}
coefplot::coefplot(lasso_cv, lambda = .7586)
```

The inputs that matter based on the tuned Lasso model are x03, x07, x16, x21, and x33.
...
```{r solution_5c}
dfB_cor <- dfB %>% select(!y) %>% cor()
corrplot::corrplot(dfB_cor)
corrplot::corrplot(dfB_cor, type = 'upper')
```

Yes, the inputs are correlated based on my visualizations.
...
```{r solutio_5d}
XBpairs <- model.matrix(y ~ .*. - 1, dfB)
ncol(XBpairs)
```
There are 45 columns in the design matrix.
...
```{r solution_5e}
XBpairs_cor <- cor(XBpairs)
corrplot::corrplot(XBpairs_cor, type = 'upper', method = 'square')
```
...
```{r solution_5f}
corrplot::corrplot(XBpairs_cor, order = 'hclust', hclust.method = 'ward.D2', method = 'square', type = 'upper')
```
...
The set of best tuning parameters (i.e., alpha = 1, lambda = 0.068707131) is entirely consistent with lasso regression (i.e., alpha = 1). The feature correlations seem to be affecting model behavior in this application, since features that are correlated with other features get turned off according to the Lasso penalty.
...
The non-zero coefficients are x1, x3, x4, x6, x8, x9, x1:x4, x1:x6, x4:x6, x4:x7, x5:x9, x6:x8, x7:x8, x7:x9, and the intercept.
...
The rankings appear consistent with the magnitude of the coefficients printed in problem 5i.
...
```{r solution_6c}
dfC %>%
  ggplot(mapping = aes(x = x, y = y, color = v)) +
  geom_point(size = 4)
```

The output to input relationship appears to vary across the levels of v. For instance, v = A seems to be a positive linear relationship between x and y, while v = C seems to be a a negative linear relationship between x and y.
...
Yes, the output to input relationship appears to vary across levels of v.
...
summary(modC_add)
```
5 coefficients are estimated. The intercept corresponds to the average response y when x = 0 and the v category is equal to A.
...
8 coefficients are estimated. The intercept corresponds to the average response y when x = 0 and the v category is equal to A.
...
```{r solution_6g}
broom::glance(modC_add) %>% bind_rows(broom::glance(modC_int)) 
```

modC_int is better according to all model metrics given by broom::glance and is consistent with the figure created in 6d.
