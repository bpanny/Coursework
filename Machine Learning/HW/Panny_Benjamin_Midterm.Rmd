...
The a shape parameter corresponds to the a-priori number of events, the b shape parameter corresponds to the a-priori number of non-events. Therefore, the number of "prior trials" or "prior targets' this specification corresponds to is 1. It represents being "uninformed" about the process for two reasons: there is only one "prior trial" rather than 10, 100, or 1000, thus our prior knowledge is very limited, and there is an equal "number" of "prior observations" of the event and non-event (i.e., a = b; events and non-events are equally probable).
...
The posterior for the unknown event probability, for each player, is a Beta distribution.

$$
p\left(\mu_j \mid \left( m, N \right)_j \right) \sim \mathrm{Beta} \left( \mu_j \mid a_{new, j}, b_{new, j} \right)
$$
...
summary_post_df_focus_from_vague <- post_df_focus_from_vague %>%
  mutate(post_avg = anew / (anew + bnew),
         post_q05 = qbeta(p = 0.05, shape1 = anew, shape2 = bnew),
         post_q95 = qbeta(p = 0.95, shape1 = anew, shape2 = bnew))
```
...
summary_post_df_focus_from_vague %>%
  ggplot(mapping = aes(x = as.factor(player_id), y = post_avg)) + 
  geom_linerange(mapping = aes(ymin = post_q05, ymax = post_q95)) + 
  geom_point(color = 'red') +
  geom_point(mapping = aes(y = num_events / num_trials), color = 'blue', alpha = 0.5)
```

There do not seem to be any players with MLEs outside the posterior uncertainty interval. Although there are about 8 players with MLEs that seem to be exactly at the lower or upper range of the posterior uncertainty interval. 

Not all players have posterior mean values that are close to the MLEs, but about 7 players have posterior means that appear to overlap with the MLEs, and the remaining players (about 8) have posterior means that are somewhat close to the MLEs.
...
```{r, solution_01g}
summary_post_df_focus_from_vague %>%
  ggplot(mapping = aes(x = as.factor(num_trials), y = post_avg)) + 
  geom_linerange(mapping = aes(ymin = post_q05, ymax = post_q95, group = player_id), position = position_dodge(0.2)) + 
  geom_point(color = 'red', mapping = aes(group = player_id), position = position_dodge(0.2)) +
  geom_point(mapping = aes(y = num_events / num_trials, group = player_id), color = 'blue', alpha = 0.5, position = position_dodge(0.2))
```

There appears to be a negative correlation between number of trials (the number of times a player is a target) and the posterior uncertainty interval. Therefore, the players with the highest posterior uncertainty on the event probability are the ones with only one trial, and the posterior uncertainty decreases with each factor increment in the number of trials. Subjectively speaking, posterior uncertainty seems quite high for trials less than or equal to 13.
...
Exclude the binomial and beta distribution coefficients since they do not include $\mu$.

$$
log \left[ p \left( \mu \mid \left( \left(m, N\right)_j \right)_{j=1}^{J} \right) \right] \propto \sum_{j=1}^J \left(\log \left[ {N_j \choose m_j} \right] + \log \left[ \mu^{m_j} \right] + \log \left[ \left( 1 - \mu \right)^{N_j - m_j} \right]\right) + \log \left[ \frac {\Gamma\left(a + b\right)}{\Gamma\left(a\right)\Gamma\left(b\right)} \right] + \log \left[\mu^{a - 1} \right] + \log \left[\left( 1-\mu \right)^{b-1} \right] = \\
\sum_{j=1}^J \left(\log \left[ \mu^{m_j} \right] + \log \left[ \left( 1 - \mu \right)^{N_j - m_j} \right]\right) + \log \left[\mu^{a - 1} \right] + \log \left[\left( 1-\mu \right)^{b-1} \right] = \\
\sum_{j=1}^J \left(m_j \log \left[ \mu \right] + \left( N_j - m_j \right) \log \left[ \left( 1 - \mu \right) \right]\right) + \left( a - 1 \right) \log \left[\mu \right] + \left( b-1 \right) \log \left[1-\mu \right] 
$$
...
$$
\sum_{j=1}^J \left(m_j \log \left[ \mu \right] + \left( N_j - m_j \right) \log \left[ \left( 1 - \mu \right) \right]\right) + \left( a - 1 \right) \log \left[\mu \right] + \left( b-1 \right) \log \left[1-\mu \right] = \\
\log \left[ \mu \right]\sum_{j=1}^J \left(m_j \right) + \log \left[ \left( 1 - \mu \right) \right]\sum_{j=1}^J\left( N_j - m_j \right) + \left( a - 1 \right) \log \left[\mu \right] + \left( b-1 \right) \log \left[1-\mu \right] = \\
\log \left[ \mu \right] \times J  \bar{m} + \log \left[ \left( 1 - \mu \right) \right] \left( J\bar{N} - J\bar{m} \right) + \left( a - 1 \right) \log \left[\mu \right] + \left( b-1 \right) \log \left[1-\mu \right] = \\
\log \left[ \mu \right] \left(J\bar{m} + a - 1 \right) + \log\left[ \left( 1 - \mu \right) \right] \left( b - 1 + J\left(\bar{N} - \bar{m}\right) \right)
$$
...
The posterior distribution of the unknown "pooled" estimated $\mu$ is a beta distribution.

$$
a_{new} = a + J\bar{m} \\ 
b_{new} = b + J\left(\bar{N} - \bar{m}\right)
$$
...
post_df_focus_pooled <- df_focus %>% 
  mutate(anew = 0.5 + nrow(df_all)*mean(df_all$num_events),
         bnew = 0.5 + nrow(df_all)*(mean(df_all$num_trials) - mean(df_all$num_events)))
```
...
All the players have MLEs outside of the posterior uncertainty interval. Perhaps one or two of the players have MLEs that are somewhat close to the posterior mean.
...
The "pooled" estimate seems incorrect for this application because players have empirical catch frequencies that are vastly different from the posterior mean of the "pooled" estimate of catch rate. Also, pooling all the players results in extremely high posterior certainty about the posterior average. To summarize, not all players are created equal in terms of catch rate, but this pooled estimate essentially assumes this, it also suggests high certainty about this incorrect assumption
...
The observed catch rates of players with the number of targets greater than or equal to 25 appears approximately normal, perhaps with some left skew, and there are no observed catch rates at the extremes (equal to 0 or 1), of which there are many observed catch rates equal to these extreme values in players with less than 25 times as targets.
...
The identified log-transformed estimates are the same; I got the same parameter estimates regardless of the initial guess.
...
The number of a-priori trials represented by my estimated hyperparameters is 23.62998, or roughly speaking, 24 trials. Interestingly enough, this is equal to the threshold number of trials we used to create the df_24 tibble.
...
The empirically derived prior distribution on the event probability overlaps well with the observed histogram of the catch rates.
...
```{r, solution_03h}
paste0("The 0.05 quantile is: ", qbeta(0.05, shape1 = ab_emp_bayes[1], shape2 = ab_emp_bayes[2]))
paste0("The 0.95 quantile is: ", qbeta(0.95, shape1 = ab_emp_bayes[1], shape2 = ab_emp_bayes[2]))
```
...
Unlike the completely pooled estimates, the posterior averages and uncertainty intervals vary from player to player and there are a few more players who have their MLEs somewhat close to their posterior average. Similar to the completely pooled estimates, some players have MLEs outside of their posterior uncertainty intervals and have MLEs that are vastly different from their posterior averages.

Unlike the completely unpooled estimates, the posterior averages and uncertainty intervals do not vary as much from player to player and there are more players who have their MLEs outside of their posterior uncertainty intervals and there are less players who have their MLEs close to their posterior means.

The empirical Bayes estimates display qualities that are "in between" the completely pooled and unpooled estimates.
...
summary_post_df_all_empbayes %>%
  ggplot(mapping = aes(y = post_avg, x = num_events/num_trials, color = num_trials)) +
...
Based on my visualizations in this exam, I believe an informative prior (the empirical Bayes prior) influences the posterior by balancing/limiting the influence of the MLE on the posterior average. That is, an informative prior "slows down" how quickly the posterior average converges to the MLE as the sample size gets larger. Similarly, an informative prior decreases our posterior uncertainty interval when sample sizes are small compared to when we do not have an informative prior. When we do not have an informative prior, small posterior uncertainty intervals are something we can only achieve with large sample sizes, but an informative prior allows us to have greater certainty even when sample sizes are smaller.
...
post_player_eval <- summary_post_df_all_empbayes %>% 
  mutate(prob_grt_67 = 1 - pbeta(q = .67, shape1 = anew, shape2 = bnew))
```
...
The top 10 players based on this criteria are listed in the first 10 rows. They all have prob_grt_67 values equal to or very close to 1. They also all have more than 100 trials and more than 100 catches. They all have an anew value greater than 100 and a bnew value greater than 25, indicating fairly high posterior certainty. All of them have posterior averages greater than 0.75 and none have 0.05 quantiles less than .71 and none have 0.95 quantiles less than .78
...
The 10 players based on this criteria are listed in the first 10 rows. The smallest number of trials associated with the 10 players with the lowest posterior probability that their catch rate is greater than 0.67 is 47
...
```{r, solution_05d}
post_player_eval %>% filter(prob_grt_67 > .75) %>% arrange(num_trials)
```

The 10 players based on this criteria are listed in the first 10 rows
...
The interpretation of this question is to find players with a high probability of catching at least 2 out of every 3 throws passed to them. This is an important question because, unlike the pooled estimate suggests, not all players are created equal when it comes to their catch rates and we also are uncertain about catch rates.
...
I would recommend players to the far right of the scatterplot depending on the team's threshold of uncertainty and threshold of posterior average. It seems to me like there are plenty of players with posterior averages greater than 0.75 that may be worth taking a risk on, for the right price.